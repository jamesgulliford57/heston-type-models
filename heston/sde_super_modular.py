...
class StochasticModel:
    def __init__(self, drift, diffusion, diffusion_prime=None, **model_params):
        """
        Class for general SDEs with built in methods to produce numerical solutions
...
'''
To add:
i) Stochastic Model should not be specific to finance or the Hesotn model! No mention of S or V. also atm urs would only work in 2D
ii) Ask chatGPT for project ideas
iii) Add time decorator
iv) Take inputs as cmd line 
v) First hitting times + other analysis 
vi) Protect whole package against non standard inputs
'''