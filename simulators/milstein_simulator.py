...
class MilsteinSimulator(Simulator):
    ...
    This class implements the Milstein method for simulating stochastic differential equations (SDEs).
    """
    def __init__(self, model, simulator_params):
        ...
        model : StochasticModel
            Stochastic model to be simulated.
    ...
    def sim_path(self, path_samples, discretisation_interval):
        ...
        discretisation_interval : float
            Time step size.
        """
        bm_samples = np.zeros((self.dim, self.discretisation_parameter))  # Initialise sample and Brownian motion arrays
        # Simulation loop
        for i in range(1, self.discretisation_parameter):
            ...
            if self.dim == 1:
                path_samples[:, i] = current_state + self.drift(current_state) * discretisation_interval + self.diffusion(current_state) * bm_step + 0.5 * self.diffusion(current_state) * self.diffusion_prime(current_state) * (bm_step ** 2 - discretisation_interval)
            else:
                path_samples[:, i] = current_state + self.drift(*current_state) * discretisation_interval + np.dot(self.diffusion(*current_state), bm_step) + 0.5 * np.einsum('ij,ijk->ik', self.diffusion(*current_state), self.diffusion_prime(*current_state)) @ (bm_step ** 2 - discretisation_interval)
