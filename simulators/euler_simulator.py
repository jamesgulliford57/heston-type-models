...
class EulerSimulator(Simulator):
    """
    Euler simulator for simulating stochastic processes.
    
    This class implements the Euler-Maruyama method for simulating stochastic differential equations (SDEs).
    """
    ...
    def sim_path(self, path_samples, discretisation_interval):
        ...
        for path_index in range(1, self.discretisation_parameter):
            ...
            if self.dim == 1:
                path_samples[:, path_index] = current_state + self.drift(current_state) * discretisation_interval + self.diffusion(current_state) * bm_step
            else:
                path_samples[:, path_index] = current_state + self.drift(*current_state) * discretisation_interval + np.dot(self.diffusion(*current_state), bm_step)
